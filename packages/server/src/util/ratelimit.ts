import { ApiError, ApiRequest, FixedWindowRateLimiter } from '@technomoron/api-server-base';
...
export function enforceFormRateLimit(
	...
	if (!decision.allowed) {
		const fastifyReply = (apireq.res as unknown as { reply?: { header(k: string, v: string): void } }).reply;
