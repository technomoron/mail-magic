---
title: Overview
description: What Mail Magic is, what problems it solves, and how it approaches email operations.
---
...
## Two Ways to Use Mail Magic

### The `mail-magic` binary

The package ships a `mail-magic` bin — a reference implementation that loads a `.env` file and starts the server
with sensible defaults. It is the fastest way to get running:

```bash
npx @technomoron/mail-magic --env ./.env
# or, after installing globally / locally:
mail-magic --env ./.env
```

This is the right choice when you just need a standalone mail service and do not need to integrate it with custom
application logic.

### Embedding as a Node.js library

`@technomoron/mail-magic` also exports its core functions so you can integrate Mail Magic directly into your own
Node.js application. The server runs on [Fastify](https://fastify.dev) and the bootstrap object gives you direct access
to the `FastifyInstance` for registering plugins, hooks, and additional routes:

```ts
import { startMailMagicServer } from '@technomoron/mail-magic'

const { server, store, vars } = await startMailMagicServer({}, {
  CONFIG_PATH: './data',
  API_TOKEN_PEPPER: process.env.MAIL_MAGIC_PEPPER!,
  SMTP_HOST: process.env.SMTP_HOST!,
})

console.log(`Mail Magic running on ${vars.API_HOST}:${vars.API_PORT}`)
```

You can add custom API routes, mount custom middleware, or supply configuration programmatically instead of through a
`.env` file. See [Custom Server](/docs/server/custom-server) for detailed examples.

## The Two Email Flows

### 1. Transactional email

Transactional email is the kind of message you send because something happened: a welcome message, a receipt, a
password reset, a notification, an alert.
...
- The OpenAPI spec shipped with the package (`docs/swagger/openapi.json`) contains the complete API contract.
- The running service can expose that contract via its Swagger/OpenAPI endpoint (when enabled with `SWAGGER_ENABLED=true`).
