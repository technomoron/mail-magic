...
description: A hands-on walkthrough building a config tree with transactional and form templates for a real domain.
---

# Template Tutorial (MyOrg)
...
Mail Magic loads configuration from the folder referenced by the `CONFIG_PATH` environment variable. Keeping your
custom assets outside the application repository makes upgrades easier.
...
> **Tip:** If you want to share partials between templates, keep file names aligned (e.g. identical `header.njk`
> content under both `form-template/partials/` and `tx-template/partials/`).

> **Assets vs inline:** Any file referenced via `asset('...')` must live under `myorg.com/assets/`. The helper
> `asset('logo.png')` will become `http://localhost:3776/asset/myorg.com/logo.png` by default. Use
...
- `API_TOKEN_PEPPER` must be set when starting the server. Tokens are stored as
  `HMAC-SHA256(token, API_TOKEN_PEPPER)` in the database, so the plaintext `token` is cleared after import.
...
Copy or design a square PNG logo and add it under the domain assets folder:
...
4. Trigger a transactional email (authenticated):
    ...
    ```
5. Submit the contact form (public endpoint):
    ```bash
...
## 7. Optional: CAPTCHA and rate limiting
